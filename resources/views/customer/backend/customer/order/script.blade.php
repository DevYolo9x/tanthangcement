    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function addSelect() {
                ...
                const $row = $('<div class="row grid grid-cols-12 gap-6 mt-3"></div>');

                // Cột sản phẩm
                const $selectCol = $('<div class="col-span-12 md:col-span-4 form-group"></div>');
                ...
                // Cột số lượng
                const $inputCol = $('<div class="col-span-12 md:col-span-6 form-group"></div>');
                const $inputLabel = $('<label class="form-label">Số lượng<sup>*</sup> <span class="badge bg-danger hidden inventory p-1 rounded-full rounded-pill text-white text-xs">(Số lượng nhỏ hơn 0)</span></label>');
                const $input = $('<input type="number" class="form-control qty" name="quantities[]" placeholder="">');

                $inputCol.append($inputLabel);
                $inputCol.append($input);

                // Cột nút xoá
                const $removeBtnCol = $('<div class="col-span-12 md:col-span-1 form-group text-sm-end"></div>');
        ...
        {{--            //elQuantity.attr('max', quantity);--}}
        {{--            //elQuantity.prev().find('.inventory').html('(Số lượng nhỏ hơn ' + quantity + ')');--}}
        {{--            //elQuantity.prev().find('.inventory').removeClass('hidden');--}}
        {{--        }--}}
        {{--    })--}}
        {{--});--}}
