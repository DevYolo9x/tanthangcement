...
if (!function_exists('getSlider')) {
	function getSlider($keyword = '')
	{
		return \App\Models\CategorySlide::select('title', 'id', 'description', 'slug')->where(['alanguage' => config('app.locale'), 'keyword' => $keyword])->with('slides')->first();
	}
}
...
if (!function_exists('getPrice')) {
	function getPrice($param = [])
	{
		...
		if (isset($param['price_contact']) && $param['price_contact'] == 1) {
			return array(
				'price_old' => '',
				'price_final' => trans('index.PriceContact'),
			...
			return array(
				...
				'price_final' => !empty($price_final > 0) ? number_format($price_final, 0, ',', '.') . 'đ' : trans('index.PriceContact'),
...
if (!function_exists('checkInventoryCheckout')) {
    function checkInventoryCheckout( $options = [] )
    {
        ...
        if( isset($options["options"]) && is_array($options["options"]) && count($options["options"]) ){
            ...
            //dd($getVersionproduct);
			if(isset($getVersionproduct) && $getVersionproduct['_stock_status'] == 1){
...
if (!function_exists('getDataJson')) {
    function getDataJson( $data = array(), $meta_key = '' )
    {
        ...
        if( count($data) > 0 || $meta_key != '' ){

            foreach( $data as $key => $val ){
                if( $val['meta_key'] == $meta_key ){
                    //dd($val['meta_value']);die;
...
if (!function_exists('showFieldOfPage')) {
    function showFieldOfPage( $page, $meta_key = '' )
    {
		$page = App\Models\Page::with('fields')->where(['alanguage' => config('app.locale'), 'page' => $page, 'publish' => 0])->select('id', 'title', 'image', 'meta_title', 'meta_description')->first();
		if( isset($page) && isset($page->fields) && count($page->fields) || $meta_key != '' ){
