...
class CategoryController extends Controller
{
    ...
    {
        // $data = \App\Models\Product::select('title', 'id', 'slug', 'catalogue', 'catalogue_id', 'price', 'price_sale', 'image_json')->where('website', 'maisononline')->get();
        ...
        //             if (!empty($image)) {
        //                 $name_gen = substr($image, strrpos($image, '/') + 1);
        ...
        // die;
        $module = $this->table;
        $data =  CategoryProduct::select('id', 'slug', 'title', 'ishome', 'highlight', 'isaside', 'view_home', 'isfooter', 'order', 'publish', 'lft', 'rgt', 'level', 'created_at', 'userid_created', 'parentid')->with('user')->with('countProduct')->where('alanguage', config('app.locale'))->orderBy('lft', 'ASC');
    ...
    public function update_view(Request $request)
    {
        ...
        $_update = [
            $request->view_name => $request->view_update
        ];
