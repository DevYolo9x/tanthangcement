...
class ProductController extends Controller
{
    ...
    {
        // $data = Product::select('title', 'id', 'slug', 'catalogue', 'catalogue_id', 'price', 'price_sale', 'image_json')->where('website', 'maisononline')->get();
    ...
    public function create(Request $request)
    {
        ...
        $tags = \App\Models\Tag::select('id', 'title')->where('module', 'products')->where('alanguage', config('app.locale'))->orderBy('order', 'asc')->orderBy('id', 'desc')->get();
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $this->table])->get();
        return view('product.backend.product.create', compact('field', 'module', 'catalogues', 'tags', 'brands', 'address', 'listTaxes', 'listAttr', 'htmlAttribute', 'getTags',  'attribute_json'));
    }
    ...
    public function edit($id)
    {
        ...
        $tags = \App\Models\Tag::select('id', 'title')->where('module', 'products')->where('alanguage', config('app.locale'))->orderBy('order', 'asc')->orderBy('id', 'desc')->get();
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $this->table])->get();
        return view('product.backend.product.edit', compact('field', 'catalogues', 'brands', 'tags', 'module', 'address', 'listTaxes', 'listAttr', 'getCatalogue', 'detail', 'htmlAttribute', 'getTags',  'attribute_json', 'product_stocks'));
    }
    ...
    public function import(\App\Services\GoogleSheet $googleSheet) 
    {
