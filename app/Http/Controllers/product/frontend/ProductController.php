...
class ProductController extends Controller
{
    ...
    public function index(Request $request, $slug = "", $id = 0)
    {
        ...
        $productSame =  $productSame->select('category_products.title as titleC', 'category_products.slug as slugC', 'products.id', 'products.description', 'products.content', 'products.code', 'products.title', 'products.image_json', 'products.image', 'products.slug', 'products.price', 'products.price_sale', 'products.price_contact');
        // $productSame =  $productSame->with('getTags');
        $productSame =  $productSame->limit(2);
        ...
        $productDeals = Product::select('id', 'slug', 'title', 'image', 'price', 'price_sale', 'price_contact', 'code', 'inventoryQuantity', 'inventory', 'inventoryPolicy')
            ...
            ->get(); */
        $ishomeProduct = Product::select('category_products.title as titleC', 'category_products.slug as slugC', 'products.id', 'products.title', 'products.image_json', 'products.image', 'products.slug', 'products.price', 'products.price_sale', 'products.price_contact')
            ->join('category_products', 'category_products.id', '=', 'products.catalogue_id')
        ...
        return view('product.frontend.product.index', compact('fcSystem',  'detail', 'seo', 'productSame', 'detailCatalogue', 'fields', 'breadcrumb', 'comment_view', 'brand', 'ishomeProduct'));
    }
}
