...
namespace App\Http\Controllers\question\backend;
...
class CategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  CategoryQuestion::where('alanguage', config('app.locale'))->with('user')->with('listQuestion')->orderBy('lft', 'ASC');
        ...
        $data =  $data->select('id', 'title', 'slug', 'userid_created', 'created_at', 'publish', 'order', 'ishome', 'isservice', 'highlight', 'isaside', 'isfooter', 'isservice', 'lft', 'rgt', 'level');
        ...
        return view('question.backend.category.index', compact('data', 'module', 'configIs'));
    }
    public function create()
    {
        ...
        return view('question.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    public function store(Request $request)
    {
        ...
        if (!empty($request->file('image'))) {
            $image_url = uploadImageNone($request->file('image'), 'danh-muc-cau-hoi');
        } else {
            $image_url = $request->image_old;
        }
        if (!empty($request->file('banner'))) {
            $banner_url = uploadImageNone($request->file('banner'), 'danh-muc-cau-hoi/banner');
        } else {
        ...
        return redirect()->route('category_questions.index')->with('success', "Thêm mới danh mục thành công");
    }
    public function edit($id)
    {
        $module = $this->table;
        $detail  = CategoryQuestion::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('category_questions.index')->with('error', "Danh mục không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('question.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('category_questions.index')->with('success', "Cập nhập danh mục thành công");
    }
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = CategoryQuestion::insertGetId($_data);
        } else {
            CategoryQuestion::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
                ...
                if( $category->parentid == 0 && $request['parentid'] > 0 ){
                    $this->Helper->insert_parent_category($id, $request['parentid']);
                } else if ( $category->parentid > 0 && $request['parentid'] > 0 ) {
                    $category_child = CategoryQuestion::where(['parentid' => $category->id, 'alanguage' => config('app.locale')])->get();
                    //dd($category_child);
                    //if( $category_child && count($category_child) < 0 ){
                        $this->Helper->change_parent_category($category->parentid, $category->id, $request['parentid']);
                    //}
                    
                } else {
                    $this->Helper->remove_parent_category($category->parentid);
                }
            }
            //thêm router
            ...
            // Cập nhật danh mục mới của danh mục cha
            //if( $request['parentid'] > 0 ){
                //$this->Helper->find_parent_category($id, $request['parentid']);
            // }
