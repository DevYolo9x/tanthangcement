...
class QuestionController extends Controller
{
    ...
    {
        $this->Nestedsetbie = new Nestedsetbie(array('table' => 'category_questions'));
    ...
    {
        $module = $this->table;
        $data =  Question::where(['alanguage' => config('app.locale')])
            ->with('user:id,name')
            ->with(['relationships' => function ($query) {
                $query->select('questions_relationships.moduleid', 'category_questions.title', 'category_questions.id')
                    ->where('module', '=', $this->table)
                    ->join('category_questions', 'category_questions.id', '=', 'questions_relationships.catalogueid');
            }])
            ->with('catalogues')
            ->orderBy('order', 'ASC')
            ->orderBy('id', 'DESC');
        ...
        return view('question.backend.question.index', compact('data', 'module', 'htmlOption', 'configIs'));
    }
    public function create()
    {
        ...
        //end tag
        $tags = Tag::select('id', 'title')->where('module', 'questions')->where('alanguage', config('app.locale'))->orderBy('order', 'asc')->orderBy('id', 'desc')->get();
        ...
        return view('question.backend.question.create', compact('module', 'htmlCatalogue', 'tags', 'dropdown', 'getTags', 'action', 'field', 'products'));
    }
    public function store(Request $request)
    {
         $request->validate([
             ...
             'slug.unique' => 'Đường dẫn câu hỏi đã tồn tại.',
             'catalogue_id.gt' => 'Danh mục là trường bắt buộc.',
         ]);
        if (!empty($request->file('image'))) {
            $image_url = uploadImageNone($request->file('image'), 'questions');
        } else {
    ...
    public function edit($id)
    {
        ...
        $module = $this->table;
        $action = 'update';
        $detail  = Question::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('questions.index')->with('error', "câu hỏi không tồn tại");
        }
        $htmlCatalogue = $this->Nestedsetbie->dropdown([], config('app.locale'));
        //tags
        $getTags = [];
        ...
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        $products = dropdown(\App\Models\Product::select('id', 'title')->where('alanguage', config('app.locale'))->orderBy('order', 'asc')->orderBy('id', 'desc')->get(), 'Chọn sản phẩm', 'id', 'title');
        return view('question.backend.question.edit', compact('module', 'detail', 'htmlCatalogue', 'tags', 'dropdown', 'getTags', 'action', 'getCatalogue', 'field', 'products', 'getProduct'));
    }
    public function update(Request $request, $id)
    {
        ...
        //upload image
        if (!empty($request->file('image'))) {
            $image_url = uploadImage($request->file('image'), 'questions');
        } else {
            $image_url = $request->image_old;
        }
        //end
    ...
    public function submit($request = [], $action = '', $id = 0, $image_url = '')
    {
        ...
        $detail = CategoryQuestion::select('id', 'title', 'slug', 'lft')->where('id', $request['catalogue_id'])->first();
        $breadcrumb = CategoryQuestion::select('id', 'title')->where('alanguage', config('app.locale'))->where('lft', '<=', $detail->lft)->where('rgt', '>=', $detail->lft)->orderBy('lft', 'ASC')->orderBy('order', 'ASC')->get();
        ...
        if (!empty($id)) {
            ...
            if ($action == 'update') {
                ...
                DB::table('questions_relationships')->where(['moduleid' => $id, 'module' => $this->table])->delete();
